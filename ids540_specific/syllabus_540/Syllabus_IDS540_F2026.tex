...
\begin{minipage}[t]{.5\textwidth}
	\textbf{Adriane Fresh} \\
	 ...
	 Office Hours: via Calendly (\url{https://calendly.com/adriane-adrianefresh/20min}) \\
     \vspace*{0.1cm}
\end{minipage}
\begin{minipage}[t]{.5\textwidth}
	\begin{flushright}  IDS 540\\
Fall 2026\\
    \vspace*{0.1cm}
\end{flushright}
\end{minipage}
...
The ideal student is someone who has taken some courses in basic probability and statistics, and has some experience working with data in a language like R or Stata, but wants to dramatically improve their ability to manipulate data and employ more sophisticated data science tools to answer questions that matter to them.  \emph{However,} zero prior programming experience is assumed!  If you don't know Stata, R, SAS, SPSS, Java, or any other programming language, this class is still for you.
...
All late assignments will be penalized 7\% per day the assignment is late, up to a maximum penalty of 50\%.
...
But it is \emph{not} a free pass to \emph{skip} an Exercise — \underline{uncompleted Exercises are not eligible for being dropped.}
...
\begin{itemize}
	...
	\item Makeups will only be offered \textbf{if} the student has reached out to me \textbf{before} class to alert me to their absence and the reason for it. If the reason is it deemed to be grounds for an excused absence, the student will be able to take the quiz at a different time.
	\item Students who are not physically in class are also \textbf{NOT allowed to take quizzes remotely without express permission.} If you are found to have taken a quiz when not in class, not only will you receive a zero on that quiz, but it may be \underline{considered an honor code violation.} Note that because gradescope tracks IP addresses, taking a quiz when not in class is not hard to ascertain.
\end{itemize}
