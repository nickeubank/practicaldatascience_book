...
\author{Team Project, Practical Data Science I (IDS 540)}
\date{\today}
...
(If this is making your head spin a little, don't worry -- this stuff is hard, and this is only a \emph{very} quick summary of causal inference.)
...
Two sources of data you will definitely need for this analysis are information on (a) opioid prescriptions, and (b) drug overdose mortality. 

\href{https://github.com/nickeubank/ids540_opioid_data}{Data we are providing you can be found here}.

You will also likely need at least one other source of data, which you will be required to find on your own.
...
You may also find it helpful to review the documentation from the Washington Post about the raw data (before it was collapsed to the county-year level) \href{https://github.com/wpinvestigative/arcos-api/blob/master/data/data_dictionary.csv}{which you can find here}.

\subsubsection*{Vital Statistics Mortality Data}

The best national source of data on drug overdoses is the US Vital Statistics records, which include data on every death in the United States. Because their interface is a \emph{pain} (e.g. you can only pull 75,000 records at a time, and picking the right variables to download is really annoying), I've pre-downloaded their summary of mortality for drug and non-drug-related causes for every US county from 2003-2015 for your use.

Several notes about this data:
\begin{itemize}
  \item I've made a couple of small changes to this data, but they are very small. This is the raw data (in the format originally provided) by the US Vital Statistics system. \textbf{So beware of formatting and cleanliness issues.}
  \item Please just count deaths where the value of ``Drug/Alcohol Induced Cause'' is ``Drug poisonings (overdose) Unintentional (X40-X44).''
  ...
  \begin{itemize}
    \item To address this problem, \textbf{you should introduce a scope condition}: a clear, principled limitation on the set of counties you are trying to analyze. In particular, you want to limit your attention to a set of counties for which this problem does not apply. Because the reporting threshold used by Vital Statistics is about the \emph{absolute} number of deaths, it is primarily a concern for small counties. Large counties will always tend to have more than 10 deaths regardless of whether they have high or low overdose rates just because of their size. Given that, you should address this problem by \textbf{dropping all counties with average populations under 350,000 people} (this is a number that was chosen empirically, as shown in Figure~\ref{fig:missing_rates} below — the rate of missing data falls off almost entirely for counties with more than 350,000 people).
    \item The consequence of this addition is that you're removed most of the bias in exchange for estimating an effect for a specific population: counties with populations over 350,000 people.
  \end{itemize}
\end{itemize}

\begin{figure}[h!]
  \centering
  \caption{}\label{fig:missing_rates}
  \includegraphics[width=0.7\textwidth]{images/missing_rates.pdf}
\end{figure}
...
\begin{itemize}
  \item An outline of your project strategy is due on November 10th.
  \begin{itemize}
  ...
  \end{itemize}
  This should include a summary of the datasets you plan to use, a summary of how you plan to merge these datasets, who will be responsible for writing initial code for each step, and who will review each set of code. \href{https://github.com/nickeubank/practicaldatascience_book/blob/main/ids720_specific/opioids/PracticalDataScience_BackwardsDesignTemplate.pdf}{You can find a rough template here if you want it.}
  \item A preliminary draft of your report will be due November 30th. \href{https://ds4humans.com/40_in_practice/27_data_science_memos.html}{Please follow these guidelines for writing up your report}.
  \item A final draft of your report will be due December 12th.
\end{itemize}

\subsection*{Tips}
\begin{itemize}
  \item Keep your datasets in a common Dropbox folder. Keep your source data in it's own folder on dropbox. You should \emph{only} ever read data from your source data, not modify it directly. This is to ensure that your source files never get corrupted.
  \item Store the files in \href{https://www.practicaldatascience.org/notebooks/PDS_not_yet_in_coursera/30_big_data/30_parquet.html}{parquet (click this link to read more)} format to minimize memory use.
\end{itemize}
